...
class MultisigAccount {
  ...
  toJSON(balance) {
    ...
    const receive = this.account.receiveAddress();
    const change = this.account.changeAddress();
    const nested = this.account.nestedAddress();
    ...
    return {
      ...
      witness,
      watchOnly,
      type: Account.typesByVal[this.account.type].toLowerCase(),
      m,
      n,
      accountIndex,
      accountPath: this.accouunt.accountPath || accountIndex,
      receiveDepth,
      changeDepth,
      nestedDepth,
      lookahead,
